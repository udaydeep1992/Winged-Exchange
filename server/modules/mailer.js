...
{
    const subject = 'Login Confirmation';

    const body = 
        "<h3>Hello "+unescape(user)+"</h3>" +
        "<p>You have recently received instructions to enter a one-time authentication code in order to log into your "+ " account. Your code is:</p>" +
        "<p>"+pin+"</p>" +
        "<p>For security reasons, this code will expire in 5 minutes.</p>" +
        "<p>If you did not request this code? you should <a href='https://"+g_constants.DOMAIN+"/profile'>change</a> or <a href='https://"+g_constants.DOMAIN+"/password_reset'>reset</a> your "+ " password immidiately.</p>" +
        "</br></br>Best Regards,<br>"+ "Wingedex Wingedex Team";
    
    try
    {
        let isSent = false;
        sendmail({
            from:  'Wingedex <'+g_constants.NOREPLY_EMAIL+'>',
...
{
    const subject = 'Signup Confirmation Letter';
...
//        "<p>Registration code is valid for 1 hour</p>" +
        ...
        "<p>Welcome to "+ "!</br></br>Best Regards,<br>"+ " Wingedex Team";
...
exports.SendPasswordResetConfirmation = function(email, user, url, urlCheck, callback)
{
    ...
    const body = 
        "<h3>Hello "+unescape(user)+"</h3>" +
        "<p>Someone requested that the password for your "+ " account be reset</p>" +
        "<p>"+confirmHREF+"</p>" +
        ...
        "<p>This is an automated message. Please, do not reply to it.</p>" +
        "</br></br>Best Regards,<br>"+ " Wingedex Team";
...
exports.SendTicket = function(ticket, callback)
{
    try
    {
        ...
        sendmail({
            ...
            subject: g_constants.EXCHANGE_NAME+' Ticket #'+ticket.id+": "+unescape(ticket.subject),
...
{
    const subject = ' Withdraw Confirmation';

    const confirmHREF = "<a href='"+urlCheck+"'>Click here to confirm withdraw</a>";

    const body = 
        "<h3>Hello "+unescape(user)+"</h3>" +
        "<p>Someone requested withdraw from your "+ " balance</p>" +
        "<p>"+confirmHREF+"</p>" +
...
exports.SendStartAppNotification = function(callback)
{
    try
    {
        ...
        sendmail({
            ...
            subject: g_constants.EXCHANGE_NAME+' Process starting notification email',
...
exports.SendAdminNotify = function(message, callback)
{
    try
    {
        ...
        sendmail({
            ...
            to: g_constants.SUPPORT_EMAIL,
            replyTo: unescape(g_constants.NOREPLY_EMAIL),
            subject: g_constants.EXCHANGE_NAME+' Admin activity notification email',
