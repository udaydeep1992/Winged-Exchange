    ...
    utils.GetSessionStatus(req, status => {
        ...
        utils.CheckCoin(unescape(req.body.coin), err => {
            ...
            g_constants.dbTables['balance'].selectAll('*', WHERE, '', (err, rows) => {
                ...
                if (fullAmount*1 < 0.00005) return onError(req, res, 'Bad order total ( total < 0.00005 ) '+'( '+fullAmount*1+' < 0.00005 )');
...
function ValidateOrderRequest(req)
{
    ...
    if (req.body.amount*1 < 0.00005)
    {
        req['message'] = 'Bad order amount ( amount < 0.00005000 ) '+'( '+req.body.amount*1+' < 0.00005000 )';
        return false;
    }
    if (req.body.price*1 < 0.00000100)
    {
        req['message'] = 'Bad order price ( price < 0.00000100 )'+' ( '+req.body.price*1+' < 0.00000100 )';
        return false;
    }
    return true;
}
...
async function AddOrder(status, WHERE, newBalance, req, res, c)
{
    ...
    require("/root/wingedex/server/modules/users/balanceupdate").UpdateBalance(status.id, unescape(req['queryCoin']), balance, "AddOrder", err => {
        if (err) 
        {
        ...
        g_constants.dbTables['referals'].selectAll('*', WHERE, '', (err, rows) => {
            ...
            for (var i=0; i<g_constants.DONATORS.length; i++)
            {
                if (g_constants.DONATORS[i].percent && g_constants.DONATORS[i].userID)
                    exports.AddBalance(g_constants.DONATORS[i].userID, (donatorsFee*(g_constants.DONATORS[i].percent*1-1)) / 100.0, price_pair, () => {}, buerID, 'From Wingedex comission');
            }
        });
    }
