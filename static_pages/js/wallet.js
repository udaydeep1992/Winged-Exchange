...
function UpdateWallet(data)
{
    ...
    const balanceLink = $('<button type="button" class="btn btn-link"></button>')
            .text((data.balance*1).toFixed(9)*1+" "+data.coin.ticker)
            .on('click', e => {
              e.preventDefault();
              ShowDetailBalanceDialog(coin, data.coin.id);
            });
          
    balanceLink.text((data.balance*1).toFixed(9)*1+" "+data.coin.ticker);
    
    if ($('#'+escape(coin).replace('%', '_')).length)
    {
      //$('#'+id_balance).text((data.balance*1).toFixed(9)*1+" "+data.coin.ticker);
...
//    const tdBalance = $('<td id="'+id_balance+'" scope="col" class="align-middle">'+(data.balance*1).toFixed(9)*1+" "+data.coin.ticker+'</td>');
    const tdBalance = $('<td id="'+id_balance+'" scope="col" class="align-middle"></td>').append(balanceLink);
    const tdAwaiting = $('<td id="'+id_awaiting+'" scope="col" class="align-middle">'+(data.awaiting*1).toFixed(9)*1+" "+data.coin.ticker+'</td>');
    const tdHold = $('<td id="'+id_onhold+'" scope="col" class="align-middle">'+(data.hold*1).toFixed(9)*1+" "+data.coin.ticker+'</td>');
...
//      message = '<div class="p-3 mb-2 bg-danger text-white">WARNING!!! ---TTC--- IS NOT TittieCoin !!!</div>';
  ...
  $.getJSON( "/api/v1/public/getmarketsummary?market="+utils.MAIN_COIN+"-"+coinTicker, ret => {
    ...
    const passwordGroup = $(
      '<div class="form-group">'+
        '<label for="id_password" class="control-label  requiredField">Wingedex password<span class="asteriskField">*</span> </label>'+
