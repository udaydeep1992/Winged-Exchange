...
function AddOrder(order)
{
  ...
  const bodyModal = 
    '<table class="table">'+
      ...
      '<tr>'+
        '<td>'+order.order+'</td>'+
        '<td>'+order.amount.toFixed(9)+'</td>'+
        '<td>'+order.coin+'</td>'+
        '<td>'+order.price.toFixed(9)+" "+MC+'</td>'+
      '</tr>'
    '</table>';
...
function UpdateMarket(message)
{
 ...
 // lastupdated = Date.now();
  ...
  for (var i=0; i<message.coins.length; i++)
  {
    ...
    const price = (message.coins[i].fromBuyerToSeller/(message.coins[i].volume == 0 ? 1 : message.coins[i].volume)).toFixed(9);
    const vol = (message.coins[i].volume*1).toFixed(9);
...
function UpdatePairData(message)
{
  ...
  if (message.data.allusers != undefined && message.data.allusers > 0)
   //chatHeader = '<span>Online: </span><strong>'+message.data.allusers;//+'</strong>&nbsp&nbsp(Registered: 417)';
// chatHeader = '<span></span><strong>'+message.data.online+'</strong>&nbsp&nbsp(Registered: '+ message.data.allusers +')';
    //'<span></span><strong>'+message.data.online+
...
function UpdateTradeHistory(history)
{
  ...
  for (var i=0; i<history.length; i++)
  {
    ...
    const tr = $('<tr></tr>')
      ...
      .append($('<td>'+volume+'</td>'))
      .append($('<td>'+price+'</td>'));
//      .append($('<td>'+(history[i].volume*1).toFixed(9)+'</td>'))
//      .append($('<td>'+(history[i].fromBuyerToSeller/history[i].volume).toFixed(9)+'</td>'));
...
function UpdateOrders(orders)
{
  ...
  {
    const price = utils.MakePrice(orders.buy[i].price);//(orders.buy[i].price*1.0).toFixed(9);
    const amountMain = utils.MakePrice(orders.buy[i].price*orders.buy[i].amount); //(orders.buy[i].price*orders.buy[i].amount*1.0).toFixed(9);
    const amountPair = utils.MakePrice(orders.buy[i].amount); //(orders.buy[i].amount*1.0).toFixed(9);
    ...
    const curVolumeBuy = (volumeBuy*1.0).toFixed(9);
    const curVolumePair = (volumeBuyPair*1.0).toFixed(9);
  ...
  $('#id_buy_volume').text(" " + (volumeBuy*1).toFixed(9)+" "+coinNameToTicker[utils.MAIN_COIN].ticker);
  
  var volumeSell = 0.0;
  var volumeSellPair = 0.0;
  for (var i=0; i<orders.sell.length; i++)
  {
    const price = utils.MakePrice(orders.sell[i].price); //(orders.sell[i].price*1.0).toFixed(9);
    const amountMain = utils.MakePrice(orders.sell[i].price*orders.sell[i].amount); //(orders.sell[i].price*orders.sell[i].amount*1.0).toFixed(9);
    const amountPair = utils.MakePrice(orders.sell[i].amount); //(orders.sell[i].amount*1.0).toFixed(9);
    ...
    const curVolumeSell = (volumeSell*1.0).toFixed(9);
    const curVolumePair = (volumeSellPair*1.0).toFixed(9);
  ...
  $('#id_sell_volume').text(" " + (volumeSell*1).toFixed(9)+" "+coinNameToTicker[g_CurrentPair].ticker);
  ...
  $('#id_max_bid').empty().append(bidButton); //.text(utils.MakePrice(orders.buy[0].price)); //((orders.buy[0].price*1.0).toFixed(9));
  $('#id_max_ask').empty().append(askButton); //.text(utils.MakePrice(orders.sell[0].price)); //((orders.sell[0].price*1.0).toFixed(9));
...
function UpdateUserOrders(userOrders)
{
  ...
  for (var i=0; i<userOrders.length; i++)
  {
    ...
    const tr = $('<tr></tr>')
      ...
      .append($('<td>'+(userOrders[i].amount*1).toFixed(9)+' '+coinNameToTicker[g_CurrentPair].ticker+'</td>'))
      .append($('<td>'+(userOrders[i].price*1).toFixed(9)+" "+coinNameToTicker[utils.MAIN_COIN].ticker+'</td>'))
...
function UpdateBalance(message)
{
  ...
  if (message.coin && (message.balance != undefined))
  {
    if (unescape(message.coin.name) == utils.MAIN_COIN)
    {
      ...
      buyBalance = (message.balance*1.0).toFixed(9);
    ...
    if (unescape(message.coin.name) == g_CurrentPair)
    {
      ...
      sellBalance = (message.balance*1.0).toFixed(9);
...
function UpdateBuyComission()
{
  ...
  try 
  {
    ...
    $('#inputBuyComission').val(comission.toFixed(9));
    $('#inputBuyTotal').val(total.toFixed(9));
...
function UpdateBuyComissionFromTotal()
{
  ...
  try 
  {
    const amount = (total / (price + utils.COMISSION*price)).toFixed(9);
    const comission = utils.COMISSION*amount*price;
    $('#inputBuyComission').val(comission.toFixed(9));
    $('#inputBuyAmount').val(amount.toFixed(9));
  }
  catch(e) {}
...
function UpdateSellComissionFromTotal()
{
  ...
  try 
  {
    const amount = (total / (price + utils.COMISSION*price)).toFixed(9);
    const comission = utils.COMISSION*amount*price;
    $('#inputSellComission').val(comission.toFixed(9));
    $('#inputSellAmount').val(amount.toFixed(9));
  }
  catch(e) {}
  //UpdateSellComission();
...
function UpdateSellComission()
{
  ...
  try 
  {
    ...
    const total = amount*price+comission;
    $('#inputSellComission').val(comission.toFixed(9));
    $('#inputSellTotal').val(total.toFixed(9));
  }
  catch(e) {}
